import { View, TouchableOpacity, Text, ScrollView, Image, SafeAreaView, StatusBar } from 'react-native';
import { useContext, useState, useEffect } from 'react';
import { MovieContext } from '../../MovieContext';
import { useNavigation } from '@react-navigation/core';
import SortModal from '../SortScreen';
import { Movie, FullMovie, imageBasePath } from '../../constants';
...
export default function SimilarMoviesScreen() {
	const { similarMovies, selectedServices, updateSimilarMovies } = useContext(MovieContext);
	...
	const getFullMovies = async () => {
		...
		updateSimilarMovies(fullMovies);
	};
		...
		if (selectedServices.length > 0) {
			if (movie.services) {
				const contains = movie.services.filter((service) =>
					selectedServices.map((service) => service.provider_id).includes(service.provider_id)
				);
	...
	return (
		<View style={{ backgroundColor: 'black' }}>
			...
			<SafeAreaView>
				<NavBar onBack={() => navigation.goBack()} onPressFilter={() => setVisible(true)} />
				<ScrollView style={styles.scrollContainer}>
					{similarMovies
						.filter((movie) => filterByService(movie))
						.sort((a, b) => compareFn(a, b))
						.slice(0, 40)
						.map((movie, index) => {
							const path = imageBasePath + movie.poster_path;
							return (
								<TouchableOpacity key={`similar-movie-${index}`} onPress={() => handlePress(movie)}>
